...
{
	pr_debug("CPU%d: %s(%d) status %08x\n",
		 smp_processor_id(), __FUNCTION__, intr, read_c0_status());
...
unsigned int gic_get_int(void)
{
	...
	pr_debug("CPU%d: %s pend=gicint%d\n",
...
static void __init gic_basic_init(int numintrs, int numvpes,
				  gic_intr_map_t *intrmap, int mapsize)
{
	unsigned int i, cpu;
