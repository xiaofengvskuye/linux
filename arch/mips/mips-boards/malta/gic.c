...
static void setup_triggers(void)
{
	...
	GIC_REG(SHARED, GIC_SH_TRIG_31_0) |=
...
static void vpe_local_setup(unsigned int numvpes)
{
	...
	for (i = 0; i < numvpes; i++) {
		...
		if (GIC_REG(VPE_OTHER, GIC_VPE_CTL) &
 ...
 * FIXME: GCMP support is separate from GIC
 */
...
static void gic_irq_init(unsigned int numintrs)
{
	...
	gic_pcpu_imasks[0] = ((1 << GIC_IPI_EXT_INTR_RESCHED_VPE0) |
			      (1 << GIC_IPI_EXT_INTR_CALLFNC_VPE0));
...
int __init gic_init(void)
{
	...
	pr_debug("GIC : %d External Interrupts Supported!\n", numintrs);
